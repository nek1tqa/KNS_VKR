...
import {checkSubgroupsArrCorrectness, checkWeekParityArrCorrectness} from "./utils";
...
export class Cell{
    ...
    constructor(lessonContainersArr, subgroupsCount, lessonNumber, groupId){
        ...
        // if(lessonContainersArr.length)
        //     this.lessonContainers = [...lessonContainersArr];
        // else
        //     // Создать массив, в котором находится только EmptyLessonContainer
        //     // wp = None
        //     // subgroups = [i+1 for i in range(subgroupsCount)]
        //     // groupId
        //     this.lessonContainers = [new EmptyLessonContainer(null, Array.from(Array.from(Array(subgroupsCount).keys()).map((val, i) => i + 1)), groupId)];
    ...
    addLessonContainer(lessonContainer = null){
        ...
        console.log(emptyLessonContainer);
    ...
    setLessonContainerPosition(LCIndex, weekParityArr, subgroups){
        ...
        // const minSubgroup = Math.min(...subgroups);
        // const maxSubgroup = Math.max(...subgroups);
        // console.log()
...
export class EmptyCell{
    ...
    getEmptyLessonContainers(){
        ...
        // Array.from(Array(this.subgroupsCount).map((val, i) => i + 1))  =====  [i + 1 for i in range(this.subgroupsCount)]
